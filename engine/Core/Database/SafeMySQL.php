...
class SafeMySQL
{
    ...
    protected $emode;
    protected $exname;
    protected $defaults = ['host' => 'localhost', 'user' => 'root', 'pass' => '', 'db' => 'basic_cms', 'port' => null, 'socket' => null, 'pconnect' => false, 'charset' => 'utf8', 'errmode' => 'exception', //or 'error'
    ...
    // * @param array $opt
    // */
    //public static function setConfig($opt = [])
    //{
    //    self::$opt = $opt;
    //}
    ...
    //        self::$instance = new self(self::$opt);
    //    }
    //
    ...
    protected function error($err)
    {
        ...
        if ($this->emode == 'error') {
            ...
            throw new ExceptionSafeMySQL($err, $this->getErrNo());
    ...
    {
        $mysqli_rollback = mysqli_rollback($this->conn);
        $this->autocommit(true);
        return $mysqli_rollback;
    }
    ...
    {
        return (int) $this->getOne("select count(*) from `?p`", $table);
    }
     ...
     * @param int      $mode
     *
     * @return void whatever mysqli_fetch_array returns
     */
    public function fetch($result, $mode = self::RESULT_ASSOC)
    {
        $array = mysqli_fetch_array($result, $mode);

        if (is_array($array)){
            foreach ($array as $key => $item){
                $array[$key] = $this->varCast($item);
            }
        }
     ...
     * @param bool   $default - optional variable to set if no match found. Default to false.
     *
     ...
     * @return array
     */
    private function cast(array $array)
    {
        $arr = [];
